...
const Calculate = () => {
  ...
  useEffect(() => {
    // 소켓 초기화
    socket.current = io(`${SERVER_WS_URL}/expenses`, {
      auth: { token }, // 인증 토큰
    });
    ...
    if (tripId && selectedDay) {
      ...
      socket.current.on("filteredExpenses", (data) => {
        // 선택한 일차의 모든 가격을 합산
        const totalPrice = data.expenses.reduce(
    ...
    socket.current.on("expenseCreated", (response) => {
      setExpenses((prevExpenses) => {
        ...
        const validExpenses = updatedExpenses.filter(
          (expense) => expense && expense.price !== undefined
        );

        // 총 경비를 계산하고 상태 업데이트
        const updatedTotal = validExpenses.reduce(
      ...
      prevExpenses.map((expense) =>
        expense.id === editingExpenseId
          ? {
              ...expense,
              ...expenseData,
              price: parseInt(expenseData.price, 10),
            } // 수정된 가격 반영
          : expense
      )
    );
    ...
    <div>
      ...
      <div className="expense-entire">
        ...
        <form
          ...
          onSubmit={
            editingExpenseId ? handleUpdateExpense : handleCreateExpense
          }
        >
          <input
      ...
      <Button customClass="total-price-button" text="총 금액 확인" />
    </div>
  );
};
